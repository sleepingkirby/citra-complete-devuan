...
inline Dest BitCast(const Source& source) noexcept {
    ...
    static_assert(std::is_trivially_copyable<Dest>(), "destination type must be trivially copyable.");
    static_assert(std::is_trivially_copyable<Source>(), "source type must be trivially copyable");
